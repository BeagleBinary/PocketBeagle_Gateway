...
void Csf_deviceSensorDisplay(ApiMac_mcpsDataInd_t *pDataInd)
{
    ...
    pBuf += 2;
    tempSensor2 = Util_buildUint16(pBuf[0], pBuf[1]);  // first sensor data
    pBuf += 2;
    lightSensor = Util_buildUint16(pBuf[0], pBuf[1]);  // first sensor data
    pBuf += 2;
    humiditySensor = Util_buildUint16(pBuf[0], pBuf[1]);  // first sensor data
    pBuf += 2;
    humiditySensor2 = Util_buildUint16(pBuf[0], pBuf[1]);  // first sensor data
    pBuf += 2;
    pBuf += 48; // skip boring stuff


    reportingInterval = Util_buildUint32(pBuf[0],
                                         pBuf[1],
                                         pBuf[2],
                                         pBuf[3]);
    pBuf += 4;
    pollingInterval = Util_buildUint32(pBuf[0],
                                       pBuf[1],
                                       pBuf[2],
				       pBuf[3]);
    ...
    pBuf += 4;
    hallEffect = (float) Util_buildUint32(pBuf[0],
                                       	  pBuf[1],
                                          pBuf[2],
				          pBuf[3]);
    ...
    if((DisplayLine_sensorStart + (pSrcAddr->addr.shortAddr - 1)) < DisplayLine_sensorEnd)
    {
        /*Board_Lcd_printf(DisplayLine_sensorStart + (pSrcAddr->addr.shortAddr - 1), "Sensor 0x%04x: Temp %d, RSSI %d, reporting interval %d, polling interval %d, hall effect %f, temperature sensor object %d, light sensor %d, humiditiy temperature %d, humidity %d",
                        pSrcAddr->addr.shortAddr, SensorData, rssi, reportingInterval, pollingInterval, hallEffect, tempSensor2, lightSensor, humiditySensor, humiditySensor2);*/
    }
    else
    {
        /*Board_Lcd_printf(DisplayLine_sensorEnd, "Sensor 0x%04x: Temp %d, RSSI %d, reporting interval %d, polling interval %d, hall effect %f, temperature sensor object %d, light sensor %d, humiditiy temperature %d, humidity %d",
                        pSrcAddr->addr.shortAddr, SensorData, rssi, reportingInterval, pollingInterval, hallEffect, tempSensor2, lightSensor, humiditySensor, humiditySensor2);*/
    }
#endif //!IS_HEADLESS
...
static bool removeDevice(ApiMac_sAddr_t addr)
{
    ...
    Csf_removeDevice(addr.addr.shortAddr);
